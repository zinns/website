...
export const buildCommentMessage = ({ comment, pull_request }: GitHubBodyRequest): string => {
...
export const buildIssueMessage = ({ action, issue, label }: GitHubBodyRequest): string => {
  switch (action) {
    case 'assigned':
...
export const buildLabelMessage = ({ action, label }: GitHubBodyRequest): string => {
  switch (action) {
    case 'created':
...
export const buildMilestoneMessage = (payload: GitHubBodyRequest): string => {
...
export const buildPullRequestMessage = ({
  ...
  switch (action) {
    ...
    case 'edited':
      return `pull request: *${pull_request?.title.toUpperCase()}* opened by *${pull_request?.user.login.toUpperCase()}* assigned to *${
        pull_request?.assignee?.login.toUpperCase() ?? 'unassigned'
      }* had edits on  ${Object.keys(changes ? changes : {})
        .join('/')
        .toUpperCase()}`;
    case 'demilestoned':
      return `pull request: *${pull_request?.title.toUpperCase()}* opened by *${pull_request?.user.login.toUpperCase()}* assigned to *${
...
export const buildPushMessage = ({
  ...
  if (commits?.length) {
    return ` *${commits?.length}* commit(s) were ${forced ? '*forced*' : ''} pushed to *${
      ref ? ref : ''.replace('refs/heads/', '').toUpperCase()
    }*`;
  }
